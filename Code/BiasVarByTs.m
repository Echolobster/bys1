function [pvalGreater, pvalLesser] = BiasVarByTs(slist,varargin)
%% TestLinearity
%
...
TAexpectation_default.tsvec = 400:10:1400;
...
for i = 1:length(slist)
    ...
    if any(isnan(simulationN))
        simulationN(i) = floor((numel(tsIn{1}) + numel(tsIn{2}))/2/length(tss));
    end
    
    TS{i} = tsIn{1}(~lapseTrials{1});
    TP{i} = tpIn{1}(~lapseTrials{1}) - mean(B(:,Fit.modelUsed));
    
    [TA{i}, TAstd{i}] = ta_expectation3(TS{i}(:),...
        wms(i),1,TAexpectation.dt,'method','numerical',...
        'trials',1,'wp',wps(i),...
        'Support',[min(tss) max(tss)],'estimator',estimator);
    
    
    [ta{i}(:,1), tastd{i}(:,1)] = ta_expectation3(TAexpectation.tsvec(:),...
        wms(i),1,TAexpectation.dt,'method','numerical',...
        'trials',simulationN(i),'wp',wps(i),...
        'Support',[min(tss) max(tss)],'estimator',estimator);
    
    for triali = 1:length(TP{i})
        res{i}(triali) = TP{i}(triali) - ta{i}(TAexpectation.tsvec(:) == TS{i}(triali),1);
    end
%     res{i} = TP{i} - TA{i}(:,1);
...
%         varianceModel(j,i,1) = mean(TAstd{i}(inds)).^2;
    ...
    TS2{i} = tsIn{2}(~lapseTrials{2});
    TP2{i} = tpIn{2}(~lapseTrials{2})  - mean(B(:,Fit.modelUsed));
    
    
    [TA2{i}, TAstd2{i}] = ta_expectation3(TS2{i}(:),...
        wms(i),2,TAexpectation.dt,'method','numerical',...
        'trials',1,'wp',wps(i),...
        'Support',[min(tss) max(tss)],'estimator',estimator);
    
    [ta{i}(:,2), tastd{i}(:,2)] = ta_expectation3(TAexpectation.tsvec(:),...
        wms(i),2,TAexpectation.dt,'method','numerical',...
        'trials',simulationN(i),'wp',wps(i),...
        'Support',[min(tss) max(tss)],'estimator',estimator);
    
    for triali = 1:length(TP2{i})
        res2{i}(triali) = TP2{i}(triali) - ta{i}(TAexpectation.tsvec(:) == TS2{i}(triali),2);
    end
%     res2{i} = TP2{i} - TA2{i};
    ...
    case {'Yes','Y','y','yes',true,1}
        ...
        for j = 1:length(tss)
            for subjecti = 1:length(slist)
                ...
                plot(squeeze(sqrt(biasSq(j,subjecti,1:2))),squeeze(sqrt(variance(j,subjecti,1:2))),'o-','Color',projectColorMaps('ts','samples',j,'sampleDepth',length(tss)),'MarkerFaceColor',projectColorMaps('ts','samples',j,'sampleDepth',length(tss)))
                hold on
            end
        end
        for j = 1:length(tss)
            for subjecti = 1:length(slist)
                subplot(ceil(sqrt(length(slist))),ceil(sqrt(length(slist))),subjecti)
                plot(squeeze(sqrt(modelbiasSq(j,subjecti,1:2))),squeeze(sqrt(varianceModel(j,subjecti,1:2))),'s-','Color',projectColorMaps('ts','samples',j,'sampleDepth',length(tss)),'MarkerFaceColor',projectColorMaps('ts','samples',j,'sampleDepth',length(tss)))
            end
        end
        for subjecti = 1:length(slist)
            subplot(ceil(sqrt(length(slist))),ceil(sqrt(length(slist))),subjecti)
        ...
        for j = 1:length(tss)
            for subjecti = 1:length(slist)
                ...
                plot(squeeze(sqrt(biasSq(j,subjecti,1)))-squeeze(sqrt(modelbiasSq(j,subjecti,1))),...
                    ...
                    'o','Color',projectColorMaps('ts','samples',j,'sampleDepth',length(tss)),...
                    'MarkerFaceColor',projectColorMaps('ts','samples',j,'sampleDepth',length(tss)))
                hold on
            end
        end
        for j = 1:length(tss)
            for subjecti = 1:length(slist)
                ...
                plot(squeeze(sqrt(biasSq(j,subjecti,2)))-squeeze(sqrt(modelbiasSq(j,subjecti,2))),...
                    ...
                    'd','Color',projectColorMaps('ts','samples',j,'sampleDepth',length(tss)),...
                    'MarkerFaceColor',projectColorMaps('ts','samples',j,'sampleDepth',length(tss)))
            end
        end
        plotVertical(0);
