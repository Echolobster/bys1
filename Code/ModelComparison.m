...
%                 LL(1:size(temp.Llikelihood,1)-1,si,modeli) = -temp.Llikelihood(1:end-1,2);
                ...
                temp = load([slist{si} '_BLSbiasedLapse_ObsAct0_20180625'],'Llikelihood');          % No CV
                LL(1:size(temp.Llikelihood,1),si,modeli) = -temp.Llikelihood(1,2);
            case 'EKF'
%                 temp = load([slist{si} '_EKF_ObsAct0_20171125'],'Llikelihood');
%                 LL(1:size(temp.Llikelihood,1)-1,si,modeli) = -temp.Llikelihood(1:end-1,1);       
                
                
                temp = load([slist{si} '_EKF_ObsAct0_20180625'],'Llikelihood');                 % No CV
                LL(1:size(temp.Llikelihood,1),si,modeli) = -temp.Llikelihood(1,1);       
            case 'BLS'
%                 temp = load([slist{si} '_BLSbiasedFitResults20150913'],'Llikelihood');
% %                 temp = load([slist{si} '_BLSbiasedLapse_ObsAct0_20170914'],'Llikelihood');
%                 LL(1:size(temp.Llikelihood,1)-1,si,modeli) = -temp.Llikelihood(1:end-1,1);     
...
%                 LL(1:size(temp.Llikelihood,1)-1,si,modeli) = -temp.Llikelihood(1:end-1,1);                
                ...
                temp = load([slist{si} '_BLS_wm1wm2_ObsAct0_20180625'],'Llikelihood');      % No CV
                LL(1:size(temp.Llikelihood,1),si,modeli) = -temp.Llikelihood(1,1);                
...
%%
figure('Name','Model comparison across subjects','Position',[501 386 956 420])

barData = permute( nanmean(LLratio,1) , [3,2,1] );
barM = mean(barData,2);
barSTE = nanstd(barData,[],2)/sqrt(size(LLratio,2));
barCI = barSTE*2;% approximation from STE; 
% barCI = permute( quantile(LLratio,[0.025 0.975],1) , [3,2,1] );
hBar = bar(1:length(Models),barM);
hold on
for ib = 1:numel(hBar)
    hBar(ib).ShowBaseLine = 'off';
    hBar(ib).EdgeColor = [0 0 0];
    hBar(ib).FaceColor = [1 1 1];
    xData = hBar(ib).XData + hBar(ib).XOffset;
    errorbar(xData',barM(:,ib)',...
        barSTE(:,ib)','o','Color',[0 0 0],'MarkerFaceColor',[0 0 0])
%     errorbar(xData',barData(:,ib)',...
%         barCI(:,ib,2)',-barCI(:,ib,1)',...
%         'o','Color',colors(ib,:),'MarkerFaceColor',colors(ib,:))
    hold on
