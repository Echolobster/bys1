function [biassquared, variance, rmse, BIASs, VARs] = RS2G_psychophysicsErrorMetrics(d,varargin)
%% RS4GPoolAnalysis
%
%   [mtp_in, stdtp_in, bias, variance, rmse, wm, wp, B, pval, weber, tsIn, tpIn, trialsIn, ts_in, tp_in, Trials_sorted, estb, llikelihood, ta, ta2] = RS2G_psychophysicsErrorMetrics(varargin)   
%
%%
...
for i = m
    ...
    for ii = 1:length(tss)
        ...
        tp_sorted{i}{ii} = tp{i}(ts{i} == tss(ii)) - b;
        Trials_sorted{i}{ii} = Trials{i}(ts{i} == tss(ii));
        
        mtp(ii,i) = nanmean(tp_sorted{i}{ii}(tp_sorted{i}{ii} >= MinMaxTp(1) & tp_sorted{i}{ii} <= tss(ii)+MinMaxTp(2)));
