...
residualVar = sqrtVAR - modelSqrtVAR;
for n = 1:size(BIAS,2)
...
for n = 1:N
    plot(sqrtVAR(:,n),modelSqrVAR(:,n),'s','Color',PlotOpts.colors(n,:),'MarkerFaceColor',PlotOpts.colors(n,:))
    hold on
end
for n = 1:N
...
mymakeaxis(gca,'xytitle',['Actual vs Circuit Model'],...
...
mymakeaxis(gca,'xytitle','Circuit Model',...
    'xticks',[-10 0 10],'xticklabels',{'-10','0','10'});

%% Residual VAR
figure('Name','Residual \sqrt{VAR}','Position',[155 296 1212 372])
edges = linspace(-10,10,8);
x = edges + (edges(2) - edges(1))/2;
