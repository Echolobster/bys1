...
%     disp('')
    ...
    if any(isnan(ta))
        ...
        for i = m
            ta(:,i) = ta_expectation3(ts_vec,wm,i,dt,'Type','N/A',...
                'method_options',method_opts,'method','numerical',...
                'trials',simtrials,'wp',0,'Support',[min(tss) max(tss)],...
                'estimator',estimator);
        end
    end
    ...
    if any(strcmp('ts_v_tp_1ax',FigureTypes))
        ...
        message = {['$$ \verb|' Fit.fittype{Fit.modelUsed} '|$$'], ...
            ...
            ['$$ w_{p_2} = ' num2str(wp2) '$$'], ...
            ['$$ \alpha = ' num2str(alpha) '$$']};
        text(500,1000,message,'Interpreter','latex')
        ...
        if length(Fit.fittype) > 1
            ...
            for i = m
                taAlt(:,i) = ta_expectation3(ts_vec,mean(WM(:,~(Fit.modelUsed == [1 2]))),...
...
%                 case {'aveMeasurements','AveMeasbiasedLapse'}
%                     for i = m
...
%                 case 'WeightedLinear'
%                     for i = m
%                         taAlt(:,i) = ta_expectation3(ts_vec,mean(WM(:,~(Fit.modelUsed == [1 2]))),i,dt,'Type','WeightedLinear','method_options',method_opts,'method','numerical','trials',simtrials,'wp',0,'Support',[min(tss) max(tss)]);
%                     end
%             end
        end
        ...
        for i = m
            if length(Fit.fittype) > 1 && ~strcmp('none',Fit.fittype(2)) && any(i == interval_N)
