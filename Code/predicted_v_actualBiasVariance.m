...
for i = 1:length(slist)
    load([slist{i} CommonFileName],'WM','WP','B','lapse','bias','BIASs','variance','VARs','rmse','RMSEs','Fit','Llikelihood','mtp_in','notFitLlikelihood','stdtp_in')
...
for i = 1:length(slist)
    for j = 1:N
        [~, ~, simbias, simv, SimRMSEBLS(i,j)] = ta_expectation3(tss',...
            wm(i,1),j,TAexpectation.dt,'method','numerical',...
            'trials',simulationN,'wp',wp(i,1),...
            'Support',[min(tss) max(tss)]);
        SimBiasBLS(i,j) = sqrt(simbias);
        SimVarBLS(i,j) = sqrt(simv);
        
        [~, ~, simbias, simv, SimRMSEAve(i,j)] = ta_expectation3(tss',...
            wm(i,2),j,TAexpectation.dt,'method','numerical',...
            'trials',simulationN,'wp',wp(i,2),...
            'Support',[min(tss) max(tss)],'Type','aveMeasurements');
...
switch TheoreticalRMSE.type
            ...
            for j = 1:N
                if ~isnan(wmvec)
                    for k = 1:length(wmvec)
                        [~, ~, ~, ~, rmseBLS(k,j,i)] = ta_expectation3(tss',wmvec(k),j,TAexpectation.dt,'method','numerical','trials',simulationN,'wp',wp(i,1),'Support',[min(tss) max(tss)]);
                        [~, ~, ~, ~, rmseAve(k,j,i)] = ta_expectation3(tss',wmvec(k),j,TAexpectation.dt,'method','numerical','trials',simulationN,'wp',wp(i,2),'Support',[min(tss) max(tss)],'Type','aveMeasurements');
                    end
                end
            end
        end
    ...
    case 'MeanWP'        
        if ~isnan(wmvec)
            for j = 1:N
                for k = 1:length(wmvec)
                    [~, ~, ~, ~, rmseBLS(k,j)] = ta_expectation3(tss',wmvec(k),j,TAexpectation.dt,'method','numerical','trials',simulationN,'wp',mean(wp(:,1)),'Support',[min(tss) max(tss)]);
                    [~, ~, ~, ~, rmseAve(k,j)] = ta_expectation3(tss',wmvec(k),j,TAexpectation.dt,'method','numerical','trials',simulationN,'wp',mean(wp(:,2)),'Support',[min(tss) max(tss)],'Type','aveMeasurements');
                end
            end
        end
    ...
    case 'MinWP'      
        if ~isnan(wmvec)
            for j = 1:N
                for k = 1:length(wmvec)
                    [~, ~, ~, ~, rmseBLS(k,j)] = ta_expectation3(tss',wmvec(k),j,TAexpectation.dt,'method','numerical','trials',simulationN,'wp',min(wp(:,1)),'Support',[min(tss) max(tss)]);
                    [~, ~, ~, ~, rmseAve(k,j)] = ta_expectation3(tss',wmvec(k),j,TAexpectation.dt,'method','numerical','trials',simulationN,'wp',min(wp(:,2)),'Support',[min(tss) max(tss)],'Type','aveMeasurements');
                end
            end
        end        
...
shortBias = sqrt((MTP(1,:,:) - repmat(permute(b(:,1),[3 2 1]),[1 2 1]) - tss(1)).^2);
longBias = sqrt((MTP(end,:,:) - repmat(permute(b(:,1),[3 2 1]),[1 2 1]) - tss(end)).^2);

% shortBias = sqrt((MTP(1,:,:) - tss(1)).^2);
% longBias = sqrt((MTP(end,:,:) - tss(end)).^2);
