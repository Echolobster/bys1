...
if length(unique(ModelUsed)) > 1
    ...
    AltModelUsed = find(1:2 ~=ModelUsed);
end
...
for i = 1:length(slist)
    for j = 1:N
        for modeli = 1:length(Models{i})
            ...
            for simi = 1:simN
            [~, ~, simbias, simv, SimRMSEtemp(simi)] = ta_expectation3(tss',...
                wm(i,modeli),j,TAexpectation.dt,'method','numerical',...
                'trials',simulationN(i),'wp',wp(i,modeli),...
                'Support',[min(tss) max(tss)],'estimator',estimator);
                ...
                if ~isnan(wmvec)
                    for k = 1:length(wmvec)
                        for modeli = 1:length(Models{i})
                            ...
                            estimator.w_int = w_int(i,modeli);
                            estimator.alpha = alpha(i,modeli);
                            [~, ~, ~, ~, rmseModel(k,j,i,modeli)] = ta_expectation3(tss',wmvec(k),j,TAexpectation.dt,'method','numerical','trials',simulationN(i),'wp',wp(i,modeli),'Support',[min(tss) max(tss)],'estimator',estimator);
                        end
                    end
                end
            end
            ...
            for j = 1:N
                for k = 1:length(wmvec)
                    for modeli = 1:length(Models{i})
                        ...
                        estimator.w_int = w_int(i,modeli);
                        estimator.alpha = alpha(i,modeli);
                        [~, ~, ~, ~, rmseModel(k,j,modeli)] = ta_expectation3(tss',wmvec(k),j,TAexpectation.dt,'method','numerical','trials',simulationN(i),'wp',mean(wp(:,modeli)),'Support',[min(tss) max(tss)],'estimator',estimator);
                    end
                end
            end
        end
            ...
            for j = 1:N
                for k = 1:length(wmvec)
                    for modeli = 1:length(Models{i})
                        ...
                        estimator.w_int = w_int(i,modeli);
                        estimator.alpha = alpha(i,modeli);
                        [~, ~, ~, ~, rmseModels(k,j,modeli)] = ta_expectation3(tss',wmvec(k),j,TAexpectation.dt,'method','numerical','trials',simulationN(i),'wp',min(wp(:,modeli)),'Support',[min(tss) max(tss)],'estimator',estimator);
                    end
                end
            end
        end        
...
shortBias = sqrt((MTP(1,:,:) - repmat(permute(b(:,ModelUsed(1)),[3 2 1]),[1 2 1]) - tss(1)).^2);
longBias = sqrt((MTP(end,:,:) - repmat(permute(b(:,ModelUsed(1)),[3 2 1]),[1 2 1]) - tss(end)).^2);
...
for modeli = 1:length(Models{1})
    [~,P(:,:,modeli),CI(:,:,modeli),STATS(modeli)] = ttest(normRMSE(:,2,modeli),1,'tail','both');
end
...
switch Plot
    case {'Yes','yes','y','Y','YES'}
        ...
        %% deltaRMSE observed v. model
        fh = figure('Name','Expected v. Actual \Delta RMSE');
        fh.Units = 'normalized';
        fh.Position = [0.0875 0.1156 0.8243 0.6178];
        edgesM = linspace(-12,12,21);
        for modeli = 1:length(Models{1})
            subplot(1,length(Models{1}),modeli)
            [~,deltaP(modeli),~,deltaSTATS(modeli)] = ...
                ttest((RMSE(:,2)-RMSE(:,1))-(SimRMSE(:,2,modeli)-SimRMSE(:,1,modeli)));
            [n, edges, ...
                dataH, patchH, zAxH, nAxH, ztextH, ntextH, zAxTickH, nAxTickH, unityH] = ...
                diagonalMarginal(RMSE(:,2)-RMSE(:,1),SimRMSE(:,2,modeli)-SimRMSE(:,1,modeli),...
                'binN',9,'T',[0.1 0.1],'nscale',8,'edges',edgesM,'offsetFactor',1,...
                'axVals',[-15 10 -15 10]);
%             plot(RMSE(:,2)-RMSE(:,1),SimRMSE(:,2,modeli)-SimRMSE(:,1,modeli),...
%                 'o','Color',[1 1 1],'MarkerFaceColor',[0 0 0]);
%             hold on
%             axis equal
%             ax(1,:) = axis;
%             xlabel('Observed \Delta RMSE (ms)')
%             ylabel(['Expected \Delta RMSE (ms) under ' Models{1}{modeli}])
%             
            dataH.MarkerFaceColor = 'k';
        end
        ...
        for modeli = 1:length(Models{1})
            ...
            ylabel(['\Delta RMSE_{' Models{1}{modeli} '} (ms)'])
            ah = gca;
            xticks = -15:5:0;
            xticklabels = {'-15','-10','-5','0'};
            yticks = -15:5:0;
            yticklabels = {'-15','-10','-5','0'};
            mymakeaxis(ah,'xytitle',Models{1}{modeli},...
                'xticks',xticks,'xticklabels',xticklabels,'yticks',yticks,'yticklabels',yticklabels)
        end
...
%         
        ...
        for modeli = 1:length(Models{1})
            ...
            hold on
%             [h, pval, CI, STATS1]= ttest(residualBias(:,1,modeli));
            text(-8,5.8,['p-val = ' num2str(pvalBias(:,1,modeli))],'Color',PlotOpts.colors(1,:));
...
%             [h, pval, CI, STATS2]= ttest(residualBias(:,2,modeli));
            text(-8,5.5,['p-val = ' num2str(pvalBias(:,2,modeli))],'Color',PlotOpts.colors(2,:));
        ...
        for modeli = 1:length(Models{1})
            ...
            hold on
%             [h, pval, CI, STATS1]= ttest(residualBias(:,1,modeli));
            text(-8,5.8,['p-val = ' num2str(pvalVAR(:,1,modeli))],'Color',PlotOpts.colors(1,:));
            ...
            barProperties.FaceAlpha = 0.3;
            mybargraph(x(1:end-1),ncount,'barProperties',barProperties);
            text(-8,5.5,['p-val = ' num2str(pvalVAR(:,2,modeli))],'Color',PlotOpts.colors(2,:));
            
            axis([-10 10 0 6])
            
            ylabel('Count')
            xlabel('Residual (VAR)^{1/2} (ms)')
            mymakeaxis(gca,'xytitle',Models{1}{modeli},...
                'xticks',[-10 0 10],'xticklabels',{'-10','0','10'});
        end
